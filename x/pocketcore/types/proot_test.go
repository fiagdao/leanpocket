...
func TestChallengeProofInvalidData_ValidateLocal(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tt.proof.ValidateLocal(h, tt.maxRelays, tt.supportedBlockchains, 5, tt.sessionNodes, tt.reporterAddress, GlobalEvidenceCacheLegacy); (err != nil) != tt.hasError {
				fmt.Println(tt.name)
				fmt.Println(err)
				t.Fatalf(err.Error())
			}
