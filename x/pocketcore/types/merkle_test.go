...
func TestEvidence_GenerateMerkleRoot(t *testing.T) {
	ClearEvidence(GlobalEvidenceCacheLegacy)
	...
	root := i.GenerateMerkleRoot(0, GlobalEvidenceCacheLegacy)
	...
	iter := EvidenceIterator(GlobalEvidenceCacheLegacy)
	// Make sure its stored in order!
	defer iter.Close()
	for ; iter.Valid(); iter.Next() {
		e := iter.Value()
		assert.Equal(t, i, e)
		newRoot := e.GenerateMerkleRoot(0, GlobalEvidenceCacheLegacy)
...
func TestEvidence_VerifyMerkleProof(t *testing.T) {
	...
	assert.True(t, res)
	index2 := 0
	root2 := i2.GenerateMerkleRoot(0, GlobalEvidenceCacheLegacy)
