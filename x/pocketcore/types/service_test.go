...
func TestRelay_Validate(t *testing.T) { // TODO add overservice, and not unique relay here
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			k3 := MockPocketKeeper{}
			_, err := tt.relay.Validate(newContext(t, false).WithAppVersion("0.0.0"), k, k2, k3, tt.node.Address, tt.hb, 1, GlobalEvidenceCacheLegacy)
...
func TestRelay_HandleProof(t *testing.T) {
	...
	validRelay.Proof.RequestHash = validRelay.RequestHashString()
	validRelay.Proof.Store(sdk.NewInt(100000), GlobalEvidenceCacheLegacy)
	res := GetProof(SessionHeader{
		ApplicationPubKey:  appPubKey,
		Chain:              ethereum,
		SessionBlockHeight: 1,
	}, RelayEvidence, 0, GlobalEvidenceCacheLegacy)
