...
func TestKeeper_GetSetClaim(t *testing.T) {
	...
	evidence, err := types.GetEvidence(header, types.RelayEvidence, sdk.NewInt(100000), types.GlobalEvidenceCacheLegacy)
	assert.Nil(t, err)
	claim := types.MsgClaim{
		SessionHeader: header,
		MerkleRoot:    evidence.GenerateMerkleRoot(0, types.GlobalEvidenceCacheLegacy),
		TotalProofs:   9,
...
func TestKeeper_GetSetDeleteClaims(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		npk, header, _ := simulateRelays(t, keeper, &ctx, 5)
		evidence, err := types.GetEvidence(header, types.RelayEvidence, sdk.NewInt(1000), types.GlobalEvidenceCacheLegacy)
		assert.Nil(t, err)
		claim := types.MsgClaim{
			SessionHeader: header,
			MerkleRoot:    evidence.GenerateMerkleRoot(0, types.GlobalEvidenceCacheLegacy),
...
func TestKeeper_GetMatureClaims(t *testing.T) {
	...
	i, err := types.GetEvidence(header, types.RelayEvidence, sdk.NewInt(1000), types.GlobalEvidenceCacheLegacy)
	assert.Nil(t, err)
	i2, err := types.GetEvidence(header2, types.RelayEvidence, sdk.NewInt(1000), types.GlobalEvidenceCacheLegacy)
	assert.Nil(t, err)

	matureClaim := types.MsgClaim{
		SessionHeader: header,
		MerkleRoot:    i.GenerateMerkleRoot(0, types.GlobalEvidenceCacheLegacy),
		TotalProofs:   9,
		FromAddress:   sdk.Address(npk.Address()),
		EvidenceType:  types.RelayEvidence,
	}
	immatureClaim := types.MsgClaim{
		...
		MerkleRoot:    i2.GenerateMerkleRoot(0, types.GlobalEvidenceCacheLegacy),
		TotalProofs:   9,
