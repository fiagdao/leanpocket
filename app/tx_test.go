		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < 5; i++ {
				...
				pocketTypes.SetProof(pocketTypes.SessionHeader{
					...
					SessionBlockHeight: 1,
				}, pocketTypes.RelayEvidence, proof, sdk.NewInt(1000000), pocketTypes.GlobalEvidenceCacheLegacy)
				assert.Nil(t, err)
			}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, c := range challenges {
				c.Store(sdk.NewInt(1000000), pocketTypes.GlobalEvidenceCacheLegacy)
			}
			_, _, evtChan := subscribeTo(t, tmTypes.EventTx)
